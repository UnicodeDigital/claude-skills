...
调用 `get_query_context()` —— 一次性拿到表列表指标口径词典 + SQL 示例。
...
5. **Symbol 规范**：内部统一小写中划线 `btc-usdt`；Hyperliquid 用大写 coin（`BTC`）。部分表同时有 `symbol`（原始如 `BTCUSDT`）和 `symbol_us`（内部 `btc-usdt`）两套字段，JOIN / WHERE 要对齐同一套。不确定时 `describe_table` 看 sample。
